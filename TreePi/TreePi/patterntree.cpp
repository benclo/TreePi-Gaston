...
PatternTree::PatternTree(Path& path, unsigned int legindex, unordered_map<Graph, int, GraphHasher>& trees) {
    PathLeg& leg = (*path.legs[legindex]);

    if (dooutput) {
        graphstate.print(trees, path.legs[legindex]->occurrences.frequency);
    }
...
PatternTree::PatternTree(PatternTree& parenttree, unsigned int legindex, unordered_map<Graph, int, GraphHasher>& trees) {
    Leg& leg = *(parenttree.legs[legindex]);

    if (dooutput) {
        graphstate.print(trees, parenttree.legs[legindex]->occurrences.frequency);
    }
...
void PatternTree::expand(unordered_map<Graph, int, GraphHasher>& trees) {
