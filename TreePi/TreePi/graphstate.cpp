...
void GraphState::print(unordered_map<Graph, int, GraphHasher>& trees, int freq) {
    ...
    for (int i = 0; i < nodes.size(); i++) {
        currentGraph.setVertexName(i, database.nodelabels[nodes[i].label].inputlabel);

        // Debugging: Print node output
        //std::cout << "v " << i << " " << database.nodelabels[nodes[i].label].inputlabel << std::endl;
    }
    ...
    for (int i = 0; i < nodes.size(); i++) {
        for (int j = 0; j < nodes[i].edges.size(); j++) {
            GraphState::GSEdge& edge = nodes[i].edges[j];
            if (i < edge.tonode) {
                currentGraph.addEdge(i, edge.tonode, to_string(database.edgelabels[database.edgelabelsindexes[edge.edgelabel]].inputedgelabel));

                // Debugging: Print edge output
               /* std::cout << "e " << i << " " << edge.tonode << " "
                    << database.edgelabels[database.edgelabelsindexes[edge.edgelabel]].inputedgelabel
                    << std::endl;*/
            }
        }
    }

    trees.insert({ currentGraph, freq });
}
