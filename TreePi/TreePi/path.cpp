...
Path::Path(Path& parentpath, unsigned int legindex, unordered_map<Graph, int, GraphHasher>& trees) {
    PathLeg& leg = (*parentpath.legs[legindex]);
    int positionshift;

    //Here I need to pass the tree to graphstate.print and do tree.insert({graph, parentpath.legs[legindex]->occurrences.frequency});

    if (dooutput) {
        graphstate.print(trees, parentpath.legs[legindex]->occurrences.frequency);
    }
...
void Path::expand2(unordered_map<Graph, int, GraphHasher>& trees) {
    ...
    if (closelegs.size() && phase > 2) {
        ...
        for (int i = 0; i < closelegs.size(); i++) {
            if (closelegs[i]->tuple.from == from &&
                ...
                if (dooutput) {
                    graphstate.print(trees, closelegs[i]->occurrences.frequency);
                }
...
void Path::expand(unordered_map<Graph, int, GraphHasher>& trees) {
